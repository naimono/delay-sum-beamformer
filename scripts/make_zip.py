...
def  main():
  ...
  installer = "\installer\DelaySum Installer.exe"
   
  if demo:
    installer = "\installer\DelaySum Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DelaySum manual.pdf" 
  ]
  ...
  ZIPNAME = "DelaySum-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DelaySum-v" + FULLVERSIONSTR + "-win-demo.zip"
